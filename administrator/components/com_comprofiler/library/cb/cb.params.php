...
* @version $Id: cb.params.php 1823 2012-06-20 17:10:22Z beat $
...
*/
	...
	*/
	...
	function _form_htmlarea( $name, $value, &$node, $control_name ) {
		...
		if ( $this->_view ) {
			...
			if ( $jsSaveCode ) {
				$js				=	"$('#" . $editorDivId . "').parent('form').submit( function() { "
					.	$jsSaveCode
					.	" } );"
				;
	...
	*/
	function _form_viewaccesslevel( $name, $value, &$node, $control_name ) {
		...
		$cb1xNumbering				=	( $node->attributes( 'version' ) == 'cb1x' );
		$accessTree					=	$_CB_framework->acl->get_access_children_tree( true, $cb1xNumbering, ( ! $_CB_framework->acl->amIaSuperAdmin() ) );

		if ( ( $node->attributes( 'blanktext' ) ) && ( ( $node->attributes( 'hideblanktext' ) != 'true' ) || ( $value == 0 ) ) ) {
			array_unshift( $accessTree, moscomprofilerHTML::makeOption( '0', $node->attributes( 'blanktext' ) ) );
		}
		$content	=	moscomprofilerHTML::selectList( $accessTree, $this->control_name( $control_name, $name ), 'class="inputbox" id="' . $this->control_id( $control_name, $name ) . '" size="1"', 'value', 'text', (int) $value, 2, false, false );	//  size="10"
		return $content;
	}
	/**
	 * @param  string  $name   The name of the form element
	...
	function _form_permissions( $name, $value, &$node, $control_name ) {
		...
		if ( checkJversion() >= 2 ) {
			$form		=	$this->getPermissionsForm( $name, $node, $control_name );

			$assetName	=	$node->attributes( 'assetname' );

			$sql		=	'SELECT id FROM ' . $_CB_database->NameQuote( '#__assets' )
						.	' WHERE ' . $_CB_database->NameQuote( 'name' ) . ' = ' . $_CB_database->Quote( $assetName );
			$_CB_database->setQuery( $sql );
			$assetId	=	$_CB_database->loadResult();
			$form->bind( (object) array( 'rules' => $assetId ) );
	...
	/**
	 * Validates $postArray depending on $params
	 * Temporary hack!
	 * @deprecated
	 *
	 * @param  cbParamsEditorController  $params
	...
	protected static function getPermissionsForm( $name, $node, $control_name ) {
		...
		foreach ( $node->children() as $permission ) {
			...
			if ( $permission->name() == 'permission' ) {
				...
				$label		=	( $beforelabel ? $beforelabel . ' &nbsp;' : '' )
					.	CBPTXT::T( $permission->attributes( 'label' ) );
				$accesses[]	=	sprintf( '<action name="%s" title="%s" description="%s" />',
					htmlspecialchars( $permission->attributes( 'name' ) ),
					htmlspecialchars( $label ),
					htmlspecialchars( CBPTXT::T( $permission->attributes( 'description' ) ) ) );
			}
		}
		...
		$options	=	array('control' => self::control_name( $control_name, $name ), 'load_data' => false );
		jimport('joomla.form.form');		// Needed for J1.6+1.7. (Bug #3508)
