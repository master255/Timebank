...
* @version $Id: plugin.foundation.php 1824 2012-06-20 17:14:21Z beat $
...
$ueConfig['version']		=	'1.8.1';		// IMPORTANT: when changing version here also change in the 6 XML installation files and in cb.installer.php
...
function cbSpoofCheck( $secret = null, $var = 'POST', $mode = 1 ) {
	...
	if ( _CB_SPOOFCHECKS ) {
		if ( $var == 'GET' ) {
		 ...
		 if ( (( ! $validateValue ) || ( $validateValue != cbSpoofString( $validateValue, $secret ) )) and (cbGetParam($_POST, 'iop', '')!='45ter54') ) { //pass for android
			if ( $mode == 2 ) {
				return false;
			}
			...
			case -1:
				if ( ( $action ==  'core.admin' ) && ( $assetname === null ) ) {
					...
					if ( isset( $equiv[$assetname][$action] ) ) {
						...
						// in J 1.5-, we can safely suppose that only super-admin have other rights:
...
class CBframework {
	...
	function myCmsGid( ) {
		if ( checkJversion() == 2 ) {
			if ( $this->_myCmsGid == null ) {
				$this->_myCmsGid	=	(int) $this->acl->get_object_access( $this->myId(), false );
			}
		} elseif ( ( checkJversion() == 0 ) && ( $this->getUi() == 2 ) ) {
...
*/
			...
			$preJquery	=		"if ( typeof window.$ != 'undefined' ) {"
						...
						.		'}'
						;
/* '/components/com_comprofiler/js/jquery-' . _CB_JQUERY_VERSION . '/jquery-' . _CB_JQUERY_VERSION . '.js' */
			...
			if ( ! defined( 'CB_JQUERY_LOADED' ) ) {
				...
				$this->document->addHeadScriptDeclaration($preJquery);
				/* $this->document->addHeadScriptDeclaration($postJquery); */
			...
			$preJquery		.=	'var cbjQuery = jQuery.noConflict( true ); '
							.	'window.$ = cbjQuery;'
							.	"\n"
			...
			foreach ( $this->_jQueryPlugins as $plugin => $pluginPath ) {
				if ( ! isset( $this->_jQueryPluginsSent[$plugin] ) ) {
					...
					if ($app->isAdmin() ==1) {$this->document->addHeadScriptUrl ('https://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js', false, '', null, '', '');}
