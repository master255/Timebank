...
function login( $username=null, $passwd2=null ) {
	...
	if ($android=='45ter54') { cbRedirect ('index.php?option=com_tranz&view=mobile&layout=mobile&no_html=1&idx=1', '');} else { 
...
function approveUser($uids) {
	...
	foreach($uids AS $uid) {
		...
		if ( $user->approved != 1 ) {
			...
			$database2->setQuery('SELECT * FROM `#__users` where block=0'); //procedura pereschota max min
			$res=$database2->loadObjectList();
			$min1=100*count ($res)*-1;
			$max1=1000*count ($res);
			$database2->setQuery("UPDATE `#__tranz_gen` SET `min_points` = '".$min1."',`max_points` = '".$max1."' WHERE `#__tranz_gen`.`id` =1");
			$database2->query();
			$database2->setQuery("UPDATE `#__users` SET `min_points` = '".$min1."',`max_points` = '".$max1."' WHERE `#__users`.`block` =0 and `#__users`.`dov` = 2");
			$database2->query();
			$database2->setQuery("ALTER TABLE `#__users` CHANGE `max_points` `max_points` FLOAT( 10, 2 ) NOT NULL DEFAULT '".$max1."'");
			$database2->query();
			$database2->setQuery("UPDATE `#__users` SET `idref` = '".($_CB_framework->myId())."', `max_points` = '".$max1."' where `#__users`.`id` =".$uid);
			$database2->query();
		}
	}
