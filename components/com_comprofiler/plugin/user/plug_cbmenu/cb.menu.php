	...
	function getMenuAndStatus( $tab, $user, $ui ) {
		...
		if ( $_CB_framework->myId() != $user->id && $_CB_framework->myId() > 0 ) {
			...
			if (count($resultArray) > 0) {
				...
				$mi = array(); $mi["_UE_MENU_MESSAGES"]["Открыть чат с пользователем"]=null;
				$this->_addMenuItem( $mi, 'Открыть чат с пользователем', '/message?option=com_tranz&view=chat&layout=chat&no=1&with='.$user->id, "", "", "", 'В чате можно общаться быстрей, чем отправляя сообщения', "" );
			}
		}
