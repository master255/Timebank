...
function uddeIMshowInbox($myself, $item_id, $limit, $limitstart, $cryptpass, $config, $filter_user, $filter_unread, $filter_flagged, $sort_mode) {
	...
	if	($config->adminignitiononly==1) {			// admin only
		if (uddeIMisAdmin($my_gid))				// call pruneMsgs, when it is an admin or superadmin
...
//	}
	...
	echo "<tr><th style='text-align:center;' class='sectiontableheader'>".$delall."</th><th class='sectiontableheader'>&nbsp;</th><th class='sectiontableheader'>"._UDDEIM_FROM.$sort_name."</th><th class='sectiontableheader'>"._UDDEIM_MESSAGE."</th><th class='sectiontableheader'>"._UDDEIM_DATE.$sort_datum."</th><th class='sectiontableheader'>&nbsp;</th><th class='sectiontableheader'>&nbsp;</th></tr>\n";
	...
	foreach($allmessages as $themessage) {
		...
		echo "<td><a href='/message?option=com_tranz&view=chat&layout=chat&no=1&with=".$themessage->fromid."'>Чат</a></td>";
		if ($config->actionicons) {
	...
	if($config->bottomlineicons) {
		...
		echo "</th><th class='sectiontablefooter'>&nbsp;</th><th class='sectiontablefooter'>&nbsp;</th><th class='sectiontablefooter'>&nbsp;</th><th class='sectiontablefooter'>&nbsp;</th><th class='sectiontablefooter'>&nbsp;</th><th class='sectiontablefooter'>&nbsp;</th></tr>\n";
	}
...
function uddeIMshowMessage($myself, $item_id, $messageid, $isforward, $cryptpass, $config) {
	...
	foreach($displaymessages as $displaymessage) {
		...
		$headerstring.="<td valign='top' width='78%'><div class='uddeim-messagefrom'>";
		if ($displaymessage->toid!=$displaymessage->fromid) { // not a copy to myself
		...
		$headerstring.="<tr>";
		$headerstring.="<td valign='bottom'><div class='uddeim-messagefrom'>".$msgnavigation."</div></td>";
	...
	if ($config->inboxlimit) {				// there is a limit for inbox + archive
		...
		if (!uddeIMisAdmin($my_gid) && (!$disablereply || ($isforward && $config->allowforwards))) {		// so the warning is only displayed when a forward or reply is possible
			// "The allowed maximum is XX."
			// $limitreached.= _UDDEIM_INBOX_LIMIT_3." ".$config->maxarchive.". ";
			$limitreached.= " "._UDDEIM_SHOWINBOXLIMIT_2." ".$config->maxarchive.").";	// (of max. )
